...
public class Cat : InteractableObject
{
    ...
    private HideoutArea? _currentTargetArea;
    ...
    {
        BepInExPlayerEvents.Instance.AreaSelected += SetTargetArea;
        BepInExPlayerEvents.Instance.AreaLevelUpdated += OnAreaLevelUpdated;
        BepInExPlayerEvents.Instance.PlayerPrepareWorkout += OnPlayerPrepareWorkout;
        BepInExPlayerEvents.Instance.PlayerStopWorkout += GoToRandomArea;
    }
    ...
    private void OnAreaLevelUpdated(HideoutArea areaData)
    {
    ...
    {
        // 4.0.13: flashlight state not easily accessible from cat; keep eyes neutral
        return false;
    }
    ...
    private bool IsBusy()
    {
        // Busy only when sleeping/eating/defecating (upstream had an off-by-comparison
        // that made Idle..Lying count as busy, freezing the cat in place)
        return _currentState >= ECatState.Sleeping;
    }
    ...
    {
        Plugin.Log!.LogInfo("GoToRandomArea: no area nodes available, falling back to closest waypoint");
    }

    public void SetTargetArea(HideoutArea area)
    {
    ...
    private void StartTraversingToArea(HideoutArea area)
    {
        _currentTargetArea = area;

        var nodes = Plugin.CatGraph!.FindDeadEndNodesByAreaTypeAndLevel(
            area.AreaTemplate.Type, 
            Plugin.GetAreaLevel(area)
        );

        if (nodes.Count == 0)
        {
            Plugin.Log!.LogInfo($"No available nodes for {area.AreaTemplate.Type} level {Plugin.GetAreaLevel(area)}");
            return;
        }
