...
namespace SPTMiniLauncher
{
    public partial class Form1 : Form
    {
        ...
        private bool launcherAutoStarted = false;
        ...
        public int fetchInstalledMods()
        {
            ...
            if (!Directory.Exists(clientModsBepinFolder))
                clientModsBepinFolder = Path.Combine(Properties.Settings.Default.server_path, "..", "BepInEx");
        ...
        public void globalProcessDetector_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (globalProcessDetector != null)
            {
                ...
                // SPT 4.0+: auto-launch SPT.Launcher when server is up and launcher not running
                bool isLauncherRunning = Process.GetProcesses().Any(p => p.ProcessName.Equals("SPT.Launcher", StringComparison.OrdinalIgnoreCase));
                if (isServerRunning && !isLauncherRunning && !launcherAutoStarted)
                {
                    string launcherExe = Path.Combine(Properties.Settings.Default.server_path, "SPT.Launcher.exe");
                    if (File.Exists(launcherExe))
                    {
                        try { Process.Start(launcherExe); launcherAutoStarted = true; }
                        catch (Exception ex) { Debug.WriteLine("Auto-launch failed: " + ex.Message); }
                    }
                }
                if (!isServerRunning)
                    launcherAutoStarted = false;
                        ...
                        "open client mods")
                    {
                        ...
                        else
                        {
                            ...
                            if (!Directory.Exists(pluginsFolder))
                                pluginsFolder = Path.Combine(Properties.Settings.Default.server_path, "..", "BepInEx\\plugins");
                            if (Directory.Exists(pluginsFolder))
                            {
                                try
                                {
                        ...
                        "open profiles folder")
                    {
                        ...
                        if (userFolderExists)
                        {
                            ...
                            if (profilesFolderExists)
                            {
                                try
                                {
                                    Process.Start("explorer.exe", profilesFolder);
                                }
                                catch (Exception err)
                                {
                                    Debug.WriteLine($"ERROR: {err.ToString()}");
                        ...
                        "open root")
                    {
                        string rootFolder = Path.GetFullPath(Path.Combine(Properties.Settings.Default.server_path, ".."));
        ...
        public void runServer()
        {
            ...
            if (Properties.Settings.Default.timedLauncherToggle)
            {
                ...
                try
                {
                    ...
                    akiServer.Start();
                    // SPT 4.0+: auto-launch the official SPT.Launcher so it waits for the server
                    try
                    {
                        string launcherExe = Path.Combine(Properties.Settings.Default.server_path, "SPT.Launcher.exe");
                        if (File.Exists(launcherExe))
                    ...
                    catch (Exception lex) { Debug.WriteLine("Auto-launch launcher failed: " + lex.Message); }
        ...
        {
            int akiPort;
            string portPath = Path.Combine(Properties.Settings.Default.server_path, "SPT_Data\\database\\server.json");
